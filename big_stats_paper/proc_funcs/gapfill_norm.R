...
gapfill_norm <- function(ts, PQ, block, pmiss){
  ...
  #-------------------------------------------------
  #Dealing with terrestrial data, since not all sites have MODIS data
  #Also includes error check if all values are the same for some reason
  #block and pmiss valuses intentionally left specified as different since 
  #MODIS only has an 8 day repeat time
  #-------------------------------------------------
    if(all(is.na(ts[, "MOD_GPP"])) == "FALSE" & length(unique(na.omit(ts[, "MOD_GPP"]))) != 1){
      #Gap-filling
        #Terrestrial GPP
    ...
    if(all(is.na(ts[, "MOD_GPP"])) == "TRUE" | length(unique(na.omit(ts[, "MOD_GPP"]))) == 1){
